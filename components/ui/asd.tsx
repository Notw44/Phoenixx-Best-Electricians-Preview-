...
// mesh-gradient: 4 points, custom gold/charcoal theme to match the app
vec3 meshGradient(vec2 uv){
  vec2 p0=vec2(-0.70,-0.45), p1=vec2(0.75,-0.35), p2=vec2(-0.65,0.65), p3=vec2(0.80,0.55);
  vec3 c0=vec3(0.06,0.05,0.02);  // deep gold-tinted charcoal
  vec3 c1=vec3(0.10,0.08,0.03);  // warm bronze-tinted slate
  vec3 c2=vec3(0.03,0.03,0.03);  // pure dark neutral/abyss
  vec3 c3=vec3(0.14,0.12,0.04);  // amber/gold highlight
  float e=2.0;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  vec3 baseDeep = vec3(0.047,0.047,0.047);
  vec3 baseTint = vec3(0.07,0.07,0.06);
  ...
  vec3 lineThin  = vec3(0.45,0.38,0.12);
  vec3 lineMajor = vec3(0.99,0.88,0.28);
  ...
  if (ASCII_AMT > 0.001) {
    ...
    vec3 asciiColor = mix(vec3(0.99,0.88,0.28), meshGradient(uv), 0.25);
